  ...
  attach(element) {
    ...
    return super.attach(element).then(() => {
      ...
      if (!bootstrapVersion(this.options)) {
        ...
        // Click event
        this.refs['sidebar-anchor'].forEach((anchor) => {
          this.addEventListener(anchor, 'click', () => {
            const clickedId = anchor.getAttribute('data-target').slice('#group-'.length);
            const clickedGroup = Array.from(this.refs['sidebar-group']).find((group) => group.getAttribute('id') === `group-${clickedId}`);
            const isOpen = clickedGroup && clickedGroup.style.display !== 'none';
            ...
            if (!isOpen) {
              Array.from(this.refs['sidebar-group']).forEach((group) => {
                const groupId = group.getAttribute('id').slice('group-'.length);
                if (groupId === clickedId) {
